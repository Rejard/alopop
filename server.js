...
app.prepare().then(() => {
  ...
  async function runStudioOrchestration(studioId, userId, task, isRevision, files = []) {
    ...
    try {
      ...
      // 💡 헌법 수호: 만약 최초 실행이나 리셋으로 인해 agentState가 비어 있다면, 템플릿의 모든 에이전트를 초기 배치 규칙에 맞게 전격 셋업
      if (Object.keys(agentState).length === 0) {
        const pipeline = template.pipeline || [];
        pipeline.forEach((name, index) => {
          const agentConf = template.agents[name] || {};
          const isDeskSeat = index < 4; // 1, 2, 3, 4번째 요원(인덱스 0~3)만 책상 배치
          
          agentState[name] = {
            status: 'idle',
            room: isDeskSeat ? 'DevRoom' : 'Conference',
            log: '',
            role: agentConf.role || '',
            expertise: agentConf.expertise || ''
          };
        });
      }
...
당신은 베타 테스터 요원 'Hank'입니다. 일반 플레이어 입장에서 이 게임을 직접 마우스나 터치로 테스트해보며 느낀 소감이나 아주 가벼운 버그성 우려사항을 재치 있고 유쾌하게 1~2문장 이내로 한국어로 작성하세요.`;
        ...
        for (let i = 0; i < pipeline.length; i++) {
          ...
          // 💡 버그 방어: 가변 인원 스튜디오(예: office)에서 이 에이전트가 현재 생성된 상태(agentState)에 포함되지 않았다면 스킵합니다.
          if (!agentState[agentName]) {
            console.log(`[runStudioOrchestration] Skipping absent agent ${agentName} for studio type ${studio.type}`);
            continue;
          }
          const agentInfo = agents[agentName];
          const currentRole = agentState[agentName].role || agentInfo.role;
          const currentExpertise = agentState[agentName].expertise || agentInfo.expertise;

          emitAgentStatus(agentName, 'thinking');
          agentState[agentName] = { 
            ...(agentState[agentName] || {}), 
            status: 'thinking', 
            // 원래 설정되어 있던 방을 유지 (점프 방지)
            room: agentState[agentName]?.room || 'DevRoom', 
            log: '업무 문서 작성 중...' 
          };
          await prisma.studio.update({ where: { id: studioId }, data: { agentStateJson: JSON.stringify(agentState) } });
          io.to(studioId).emit('syncStudioAgentState', agentState);

          await broadcastStudioLog({ agent: agentName, msg: `${currentRole} 업무 처리를 시작합니다...` });

          let prompt = `[역할: ${currentRole} | 전문성 및 페르소나: ${currentExpertise}] 당신은 이 전문성과 역할을 바탕으로 이번 업무 중 본인의 분야를 담당해 주어야 합니다.
당신은 ${template.name} 소속의 ${currentRole} '${agentName}'입니다. 
대표님의 핵심 지시: "${task}".
...
위 자료와 함께 첨부된 문서(PDF) 및 이미지 파일이 있다면 그 내용을 적극적으로 분석하여, 당신의 전문 역할(${currentRole})과 전문성(${currentExpertise})에 맞는 새로운 분석 결과, 제안, 혹은 구체적인 계약서/기획서 단락을 마크다운(Markdown) 포맷으로 추가 및 확장해 최종 문서를 빌드해 주세요.
...
[주의사항(CRITICAL)]: 절대로 완전히 새로운 게임 코드를 작성하지 마십시오. 오직 발견된 에러를 수정하기 위해 기존 코드를 최소한으로만 변경(패치)하십시오.`;
    ...
    try {
      ...
      const systemStudios = [
        {
          ...
          agentStateJson: JSON.stringify({
            "Alice": { "status": "idle", "room": "DevRoom", "log": "", "role": "기획", "expertise": "레트로 게임 디렉터 및 시나리오 라이터" },
            "Carol": { "status": "idle", "room": "DevRoom", "log": "", "role": "디자인", "expertise": "네온 스타일 전문 화려한 그래픽 디자이너" },
            "Bob": { "status": "idle", "room": "DevRoom", "log": "", "role": "개발", "expertise": "HTML5 기반 물리 엔진 특화 풀스택 엔지니어" },
            "Dave": { "status": "idle", "room": "DevRoom", "log": "", "role": "QA", "expertise": "칼 같은 엄격함을 가진 버그 헌터 QA 마스터" }
          }),
        ...
        },
        {
          ...
          currentProjectJson: JSON.stringify({ active: false, specDoc: '', designDoc: '', codeDoc: '', url: '', gameName: '' }),
          agentStateJson: JSON.stringify({
            "오기획": { "status": "idle", "room": "DevRoom", "log": "", "role": "총괄", "expertise": "콘서트 페스티벌 기획 총괄 디렉터" },
            "한재무": { "status": "idle", "room": "DevRoom", "log": "", "role": "재무", "expertise": "수지타산 및 티켓 가격 산정 재무관" },
            "윤홍보": { "status": "idle", "room": "DevRoom", "log": "", "role": "마케팅", "expertise": "온오프라인 바이럴 광고 마케팅 분석관" }
          }),
        ...
        },
        {
          ...
          currentProjectJson: JSON.stringify({ active: false, specDoc: '', designDoc: '', codeDoc: '', url: '', gameName: '' }),
          agentStateJson: JSON.stringify({
            "지분석": { "status": "idle", "room": "DevRoom", "log": "", "role": "분석관", "expertise": "사례 및 정밀 대조 판례 분석관" },
            "임변호": { "status": "idle", "room": "DevRoom", "log": "", "role": "변호사", "expertise": "종합 법률 자문 총괄 변호사" },
            "서기록": { "status": "idle", "room": "DevRoom", "log": "", "role": "기록관", "expertise": "무결한 기록 및 자문서 초안 작성 서기" }
          }),
        ...
        },
        {
          ...
          currentProjectJson: JSON.stringify({ active: false, specDoc: '', designDoc: '', codeDoc: '', url: '', gameName: '' }),
          agentStateJson: JSON.stringify({
            "김장부": { "status": "idle", "room": "DevRoom", "log": "", "role": "기장", "expertise": "매출/매입 세부 장부 기록 및 전표 입력 기장사" },
            "이절세": { "status": "idle", "room": "DevRoom", "log": "", "role": "세무", "expertise": "합법적 세액 감면 및 절세 시나리오 도출 세무사" },
            "박감사": { "status": "idle", "room": "DevRoom", "log": "", "role": "감사", "expertise": "세무 감사 리스크 진단 및 사전 감리 회계사" },
            "정신고": { "status": "idle", "room": "DevRoom", "log": "", "role": "신고", "expertise": "종합소득세, 부가가치세 및 법인세 신고 전문 세무사" },
            "최재무": { "status": "idle", "room": "Conference", "log": "", "role": "CFO", "expertise": "재무제표 최종 마감 및 경영 전략 자문 수석재무관" }
          }),
          welcomeMsg: '📊 세무회계법인 알로팝에 오신 것을 환영합니다! 대표님, 매출/매입 정산이나 세무 감사 리스크 진단 등 어떤 세무 회계 자문이 필요하십니까?'
        },
        {
          name: '일반 사무직 회사',
          type: 'office',
          ...
          currentProjectJson: JSON.stringify({ active: false, specDoc: '', designDoc: '', codeDoc: '', url: '', gameName: '' }),
          agentStateJson: JSON.stringify({
            "최인사": { "status": "idle", "room": "DevRoom", "log": "", "role": "인사", "expertise": "인사 고과 및 핵심 인재 영입 담당 시니어 인사관" },
            "정기획": { "status": "idle", "room": "DevRoom", "log": "", "role": "기획", "expertise": "차년도 사업 전략 및 경영 혁신 기획 파트장" },
            "홍홍보": { "status": "idle", "room": "DevRoom", "log": "", "role": "홍보", "expertise": "바이럴 마케팅 및 브랜드 커뮤니케이션 스페셜리스트" },
            "윤재무": { "status": "idle", "room": "DevRoom", "log": "", "role": "재무", "expertise": "자금 포트폴리오 자본 배분 및 예산 통제 CFO" }
          }),
          welcomeMsg: '🏢 일반 사무직 회사 스튜디오에 오신 것을 환영합니다! 대표님, 오늘 어떤 부서 협업 기획이나 보고서 작성 업무를 시작해볼까요?'
        }
      ];
      ...
      for (const studioData of systemStudios) {
        ...
        const exist = await prisma.studio.findFirst({
          where: { type: dbData.type, isSystem: true, ownerId: String(userId) }
        });
  ...
  aistudioRouter.post('/create', async (req, res) => {
    ...
    try {
      ...
      // agentsConfig를 바탕으로 초기 agentStateJson 구조 생성 (1~4번은 DevRoom, 5번 이후는 Conference)
      let agentState = {};
      const { agentsConfig } = req.body;
      if (Array.isArray(agentsConfig) && agentsConfig.length > 0) {
        agentsConfig.forEach((agent, idx) => {
          if (agent.name) {
            agentState[agent.name] = {
              ...
              log: '',
              role: agent.role || '사무',
              expertise: agent.expertise || '일반 사무 전문가'
            };
          }
        });
      }
